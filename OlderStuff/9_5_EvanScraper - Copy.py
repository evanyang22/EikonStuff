...
#New csv paths
#csv_path=r'C:\Users\WillKnight\Dropbox\EikonScraper\january\newcsv.csv'
csv_path=r'C:\Users\WillKnight\Documents\EikonScraper_Backup\Output_CSV_Files\221117 newcsv.csv'
#skipped_csv_path = r'C:\Users\WillKnight\Dropbox\EikonScraper\january\skippedcsv.csv'
skipped_csv_path=r'C:\Users\WillKnight\Documents\EikonScraper_Backup\Output_CSV_Files\221117 skippedcsv.csv'
log_path = r'C:\Users\WillKnight\Dropbox\EikonScraper\january\EvanCombinedCompanies.log'

# old paths below- Evan Yang
#csv_path = r'C:\Users\WillKnight\Dropbox\january\newcsv.csv'
#skipped_csv_path = r'C:\Users\WillKnight\Dropbox\january\skippedcsv.csv'
#log_path = r'C:\Users\WillKnight\Dropbox\january\EvanLog3.log'
####--------
...
#9/8/2022 Evan Combining list
df1 = pd.read_csv("master_not_in_omersp1500.csv")
df2=pd.read_csv("updated_1500.csv")
framesEvan = [df1, df2]
df = pd.concat(framesEvan)
print(len(df.index))
...
#original accounts below
#9/17/2022 new accounts below, every acccount except for 12 bc password lock
accounts = ['gbsstudent1@emory.edu', 'gbsstudent2@emory.edu','gbsstudent3@emory.edu', 'gbsstudent4@emory.edu','gbsstudent5@emory.edu', 'gbsstudent6@emory.edu', 'gbsstudent7@emory.edu', 'gbsstudent8@emory.edu','gbsstudent9@emory.edu', 'gbsstudent10@emory.edu','gbsstudent11@emory.edu', 'gbsstudent13@emory.edu','gbsstudent14@emory.edu', 'gbsstudent15@emory.edu']

#accounts after login fail below
#8/28/2022- correct accounts below
#9/17/2022 old accounts below
#accounts = ['gbsstudent1@emory.edu','gbsstudent4@emory.edu', 'gbsstudent6@emory.edu', 'gbsstudent10@emory.edu','gbsstudent13@emory.edu','gbsstudent14@emory.edu', 'gbsstudent15@emory.edu']

##################### EVan EDits ----------------------------------------------------
from time import strftime
#old- account_usage_path = r'C:\Users\WillKnight\Dropbox\january\eikon_account_usage.csv'
#account_usage_path = r'C:\Users\WillKnight\Dropbox\EikonScraper\january\eikon_account_usage.csv'
account_usage_path=r'C:\Users\WillKnight\Documents\EikonScraper_Backup\Output_CSV_Files\221117 eikon_account_usage.csv'
def getDataSize():
    # assign size
    size = 0
    # assign folder path
    Folderpath="C:\\Users\\WillKnight\\Desktop\\Eikon2022"
    # get size
...
def getFolderDataSize(Cusip,Tic): #cusipYear = Cusip + "_" + Tic
    # assign size
    size = 0
    # assign folder path
    Folderpath="C:\\Users\\WillKnight\\Desktop\\Eikon2022\\"
    cusipYear = Cusip + "_" + Tic
    Folderpath= Folderpath+cusipYear
    # get size
    for path, dirs, files in os.walk(Folderpath):
        for f in files:
            fp = os.path.join(path, f)
...
def writeUsage(accountEmail,dataUsage): # works
    with open(account_usage_path,"a",newline="") as csv_file:
        fieldnames_usage=["account_num","date","data_usage","month"]
        dateEY = strftime("%m/%d/%Y")
        current_month = strftime('%B')
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames_usage)
        writer.writerow({'account_num': accountEmail,'date':dateEY,'data_usage':dataUsage,'month':current_month})

def checkUsage(current_number): #returns the usage amount for the current month
    UsageAmount=0
    EYEmail=accounts[current_number]
    current_month = strftime('%B')
    with open(account_usage_path,"r") as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        usageData=list(csv_reader)
        for row in usageData:
            if row[0]==EYEmail and current_month==row[3]:
                UsageAmount+=float(row[2])
        print(str(EYEmail) + " has used GB " + str(UsageAmount))
        return UsageAmount
...
#evan change below 8/13/2022
incorrectLoginError=0
def main():
    global company,year,month
    global accounts
    #Evan- start Parameters
    company=8284 #Evan- change company here, companyChange
    year = 10 #Evan- Changed from 0 to 1 to do 2013
    month = 0
    report_number = 0
    global current_account, companyErrorNumber

    current_account = change_account(0)
    login(current_account) #loads account
    #Evan- open csv file and read it, check data usage-------------------------------------
    #if "skip", then just login to another account
    ...
    #---------- Evan- check and store gigs of beta test ?
    EYTotalSize=0
    #-----------------------
    while year < 11:
        global currentYear
        ...
        while company < 9191:
            ...
            #Evan EDits
            EYstartSize=getFolderDataSize(currentCusip,currentTic)
            startSearchTime=time.time()
            logging.info("Search Start Time: "+ time.asctime())
            companyErrorNumber = 0
            #evan Edi below 8/13/2022
            year_files_in_search=-1
            while True:
                try:
                    time.sleep(speed)
                    clear()
                    ...
                    if(companyErrorNumber > 2):
                        ...
                        #companyErrorNumber = 0
                        break
            logging.info("Search End Time: " + time.asctime())
            logging.info("Search Time Elapsed: "+ str(time.time()-startSearchTime) + " seconds")
            companyErrorNumber = 0
            downloadStartTime=time.time()
            logging.info("Download Start Time: "+ time.asctime())
            if year_files_in_search == -5: #If in the yearly search theres no reports available, logs none and skips the year
                ...
                logging.info("There were "+ str(year_files_in_search) + " reports")
                ...
                while quarter < 4:
                    ...
                    while True:
                        try:
                            ...
                            logging.error("Quarterly Error: "+ str(e))
                            cleanup(currentCusip, currentYear, currentTic)
                            newDriver()
                            login(change_account(current_account))
                            if(companyErrorNumber > 2):
                        ...
                        while month < plusThree :
                            ...
                            while True:
                                try:
                                    ...
                                    companyErrorNumber = companyErrorNumber + 1
                                    logging.error("Crashed on Search:" + currentTic + " - " + currentCusip + " - " + start + " through " + end)
                                    logging.error("Monthly Error: "+ str(e))
                                    cleanup(currentCusip, currentYear, currentTic)
                                    newDriver()
                                    login(change_account(current_account))
                                    if(companyErrorNumber > 2):
                                        skip_company()
                                        #companyErrorNumber = 0 I think its this line
                                        break
                            while True:
                                try:
                                    place_in_directory(currentCusip, currentYear, currentTic, start, end)
            ...
            logging.info("Download End Time: "+ time.asctime())
            logging.info("Download Time Elapsed: "+ str(time.time()-downloadStartTime) + " seconds")
            #EVAN
            EYendSize=getFolderDataSize(currentCusip,currentTic)
            EYTotalSize=EYTotalSize+EYendSize-EYstartSize
            #

            company = company + 1

            ############################# Evan edits- need to check and restore gigs of beta test\
            # need to reupdate csv file
            if(company % 100 == 0): #Changed from 25 to 40- Evan Yang
                logging.info(str(accounts[current_account])+ " Data Usage: "+str(EYTotalSize))
                writeUsage(str(accounts[current_account]),EYTotalSize)
                logging.info("Finished with 100 Searches, Switching Accounts and Drivers to Balance Load")
                driver = webdriver.Chrome(chromedriver, chrome_options=chrome_options) #makes a new driver
                login(change_account(current_account))
                EYTotalSize=0 #resets total size
                #logging.info("Switched to account number" + str(accounts[current_account]))
        ...
        year = year + 1
        company=0#reset companies- EVan Yang edit 6/6
        logging.info("Finished with the Year: " + str(currentYear))
        print("Done with year 2017, breaking")
        break
        time.sleep(speed)
...
def login(accountNumber):
    ...
    while True:
        try:
            ...
            try:
                ...
                #8/28- evan edit
                checkEvanResearch = driver.current_url
                print(checkEvanResearch)
                find_research = checkEvanResearch.find("auth")
                print(find_research)
                if find_research==-1:
                    return
                    #
    ...
    try:
        WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.ID,'AAA-AS-SI2-SE009'))).click()
    except TimeoutException:
        logging.info("Error:  Account Already in Use Popup, breaking instead") #Evan- changed from debug
        #pdb.set_trace()
    check = driver.page_source
    error_logging = check.find("Incorrect User")
    if error_logging > 0:
        login(change_account(accountNumber))
        #Evan changes 8/13/2022
        global incorrectLoginError
        incorrectLoginError+=1
        if incorrectLoginError==3:
            print("sleeping for one hour")
            time.sleep(3600)
        if incorrectLoginError==6:
            print("sleeping for 1 day")
            time.sleep(86400)
        if incorrectLoginError==9:
            pdb.set_trace()
        return
    time.sleep(5)
    ...
    current_account=accountNumber# Evan Yang Edit
...
def change_account(current_Number):
    global accounts
    global current_account #Evan Yang
    while True:
        ...
        amountUsed=checkUsage(newNum) #EY Edits
        if newNum != current_Number and int(amountUsed)<=7:
            print(str(accounts[newNum]) + " GB Usage: " + str(amountUsed))
            break
    current_account=newNum#Evan Yang Change
    return newNum
def clear():
    time.sleep(10)
    clear_error = 0
    ...
    while True:
        try:
            WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.ID, 'clearButton'))).click()
            break
        except TimeoutException:
            clear_error = clear_error + 1
            if(clear_error > 2):
...
def search_and_download(Cusip, Year, Tic, startDate, endDate):
    ...
    time.sleep(10)
    checkEvan=""
    for elemEY in driver.find_elements_by_xpath("/html/body/app-root/app-main-view/div/div[1]/amber-appstate-bar/span"):
        checkEvan+=elemEY
    print(checkEvan)
    find_old = checkEvan.find("previous design")
    print(find_old)
    if find_old > 0:
        try:
            WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, '/html/body/app-root/app-main-view/div/div[1]/amber-appstate-bar/a'))).click()
            print("Click to old page successful")
        except:
            print("Clicking to old page not successful")
...
def place_in_directory(Cusip, Year, Tic, startDate, endDate):
    ...
    helper = excel_folder = os.path.join(desktop_directory, "Eikon2022", cusipYear, yearString[2:4])
    excel_folder = os.path.join(desktop_directory, "Eikon2022", cusipYear, yearString[2:4], excelName)
    ...
    for i in range(0, 12):
        ...
        for name in os.listdir(r'C:\Users\WillKnight\Downloads'):
            ...
            if(name.startswith(start_of_file)):
                time.sleep(.01)
                fileDestination = os.path.join(desktop_directory, "Eikon2022", cusipYear, yearString[2:4], convert7(months[i]))
...
def cleanup(Cusip, Year, Tic):
    try:
        ...
        folderName = os.path.join(desktop_directory, "Eikon2022", cusipTic, yearString)
